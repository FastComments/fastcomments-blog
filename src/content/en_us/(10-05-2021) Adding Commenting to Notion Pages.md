...
FastComments is now an embed.ly provider, and as such offers APIs and services that allow comment threads
to be embedded into applications that support [oEmbed](https://oembed.com).

This means that you can now easily add live commenting to a site built with Notion, Google Sites, or Nimbus Note.

## How to Use It - A Quick Demo

When logged in to your tenant, you can simply visit [fastcomments.com/create](https://fastcomments.com/create) to create a conversation link that can then
be added to your Notion page or any application that integrates with embed.ly.
...
For example, as shown in the above video, we can customize the comment widget so that the user only has to enter their name, not their email, to comment or vote.
