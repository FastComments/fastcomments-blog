...
It's simply a matter of removing the small snippet Muut provides and replacing it with <a href="https://fastcomments.com/auth/my-account/get-acct-code" target="_blank">our own</a>.
You can even run Muut and FastComments on the same site temporarily to test and tweak the look and feel. <a href="https://fastcomments.com/auth/my-account/help" target="_blank">We can assist with that</a>.
{{/unless}}
...
To export your site's Comment and User data from Muut, go to their Admin dashboard, then Integrations, and select JSON Export.

You'll notice that the resulting file you receive via email has a mysterious "gz" file extension. If you're a bit more technically savvy, you might know this stands for "gzip" and is a popular and efficient way to compress files. 
...
You'll need to enter the URL of your website. This is because Muut does not provide that information in the export.
...
It's safe to reimport as many times as needed - however, if your import fails, then <a href="https://fastcomments.com/auth/my-account/help" target="_blank">reach out to us</a> so we can assist.
...
comments, then <a href="https://fastcomments.com/auth/my-account/help" target="_blank">let us know</a> by sending us the old and new URLs. The help page allows for uploading small text files, so if
you are migrating more than just a few URLs, create an Excel sheet.

### Recap

1. Export your data
2. Import into FastComments
3. Replace Muut JavaScript snippet with <a href="https://fastcomments.com/auth/my-account/get-acct-code" target="_blank">FastComments's</a>

## Why Won't Muut Send Me My File?
