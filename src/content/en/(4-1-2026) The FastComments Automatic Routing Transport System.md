...
with global write availability, which allows our FARTS to be eventually consistent. [More details here](https://blog.fastcomments.com/(03-29-2026)-fastcomments-is-ready-for-space.html).
...
Part of the motivation of creating FART was replacing some existing services written in Java. After sitting on it for a while, we decided to consolidate these into one Rust
service to lower runtime overhead. This was an acceptable tradeoff since we don't deploy FART often. FART is compiled with LTO, so we can really let it rip.
...
system gives us increased visibility into latency across the system, as well as lower maintenance burden so we can now spend more time on features.
