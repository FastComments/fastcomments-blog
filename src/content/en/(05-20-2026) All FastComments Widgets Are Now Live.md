...
{{#unless isPost}}
Every embedded FastComments widget (comment counts, recent comments, top pages, recent discussions, reviews summary, floating likes) now update in real time.
{{/unless}}
...
The main comment widget has been live since day one. The smaller embedded widgets, though, were not. Comment counts, "recent comments" or "top pages" widgets, etc, would happily show a number that was up to a minute stale.

Now every widget FastComments ships now subscribes to live updates and refreshes right away :) This includes review summaries as well!

### Which Widgets

All of them. Concretely:
...
The old `isLive: true` config flag on the comment-count widgets is now redundant - widgets are always live.
...
The server-side caches that back these widgets used to expire on a 60-second TTL. They now invalidate the moment a relevant comment or reaction event lands, so the first request after a change returns fresh data, not the cached version.
