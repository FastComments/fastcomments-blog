...
{{#unless isPost}}
Ovde ćemo istražiti kako da kreiramo nit komentara koja liči na onu na Hackaday.

Ako kliknete na link do ovog posta, celokupna stilizacija stranice kao i FastComments blog će se promeniti.
...
Ovaj post je demonstracija korišćenja FastComments za pravljenje sistema komentara koji liči na https://hackaday.com.

Ne sugerišemo da ih kopirate. Ali ovo pokazuje fleksibilnost FastComments.

Učiniti da FastComments izgleda ovako je jednostavno!
Napravili smo UI za to! Na primer, evo konfiguracije koju smo koristili da uskladimo FastComments sa ovom stranicom:
...
Ako želite da to uradite u kodu, isto je jednostavno.

Samo je potrebno da postavimo nekoliko opcija. Pa, sedam. Evo snipeta koda (prilagođeni CSS je izbačen). Samo određena količina CSS-a može biti umetnuta na ovaj način. Preporučujemo da koristite
...
document.getElementById('code').innerText = `
    window.FastCommentsUI(document.getElementById('fastcomments-widget'), {
        tenantId: 'demo',
        commentCountFormat: '[count] MISLI NA "KAKO NAPRAVITI SISTEM KOMENTARA POPUT HACKADAY.COM"',
        commentCountUpdated: function(count) {
            document.getElementById('post-header-comment-count').innerHTML = Number(count).toLocaleString() + ' Komentara';
        },
        headerHTML: '<h2>Ostavite odgovor</h2>',
        hideAvatars: true,
...
Šta znače svaka od ovih stavki?
...
vrhu stranice.
- headerHTML = HTML koji se prikazuje iznad kutije "ostavite odgovor". U ovom slučaju koristimo da umetnemo element zaglavlja.
- hideAvatars = Hackaday nema avatare. Zato smo ovo postavili na true da ih sakrijemo.
- inputAfterComments = Premestražuje kutiju za odgovor na dno niza komentara.
- absoluteDates = Menja podrazumevani format datuma (npr., "Pre 11 minuta") na apsolutni datum.
